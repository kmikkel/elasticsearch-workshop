...
def before_all(context):
    ...
    with open('mapping.json') as data_file:
        ...
        app.elasticsearch.indices.delete(index='big-one', ignore=[400, 404])
        app.elasticsearch.indices.create(index='big-one')
        app.elasticsearch.indices.put_mapping(index="big-one", doc_type="pizza", body=mapping)
    ...
    with open('data.json') as data_file:
        ...
        for entry in data:
            app.elasticsearch.index(index="big-one", doc_type="pizza", id=entry['id'], body=entry)
        app.elasticsearch.indices.refresh(index='big-one')
